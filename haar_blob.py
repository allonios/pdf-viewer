...
class HaarCascadeBlobCapture:
    ...
    def blob_track(self, img, threshold, prev_area):
        Kernal = numpy.ones((3, 3), numpy.uint8)  # Declare kernal for morphology
        ...
        cv2.imshow("tresholding",img)

        img = cv2.erode(img, None, iterations=2)
        cv2.imshow("eroded threshold",img)
        img = cv2.dilate(img, None, iterations=4)
        cv2.imshow("dialate eroded threshold",img)
        img = cv2.medianBlur(img, 5)
        cv2.imshow("blur dialate eroded threshold",img)
